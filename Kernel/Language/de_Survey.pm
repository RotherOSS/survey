...
sub Data {
    ...
    $Self->{Translation}->{'If this regex matches, no customer survey will be sent.'}
        = 'Wenn dieser reguläre Ausdruck zutrifft, wird keine Umfrage an den Kunden gesendet.';
    $Self->{Translation}->{'Limit.'} = 'Grenzwert.';
    ...
    $Self->{Translation}->{
        ...
        =
        'Ergebnisse, die älter als die konfigurierte Anzahl von Tagen sind, werden gelöscht. Hinweis: Löschen Sie die Ergebnisse des OTOBO-Daemons, bevor Sie die Einstellung \'Task#####SurveyRequestsDelete\' aktivieren.';
